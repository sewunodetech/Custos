...
import { HISTORY, HF_TREND } from "@/lib/mock-data";
...
export default function HistoryPage() {
  ...
  // HF recovery bars: before vs after per execution
    ...
    <div className="w-full p-5 md:p-7 space-y-6">
      ...
      <div>
        ...
        <p className="text-[12px] text-white/30 mt-0.5">
          All Custos interventions and monitoring events
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: "Avg gas per tx",    value: "0.00038 ETH",               icon: Filter },
        ].map((s) => (
      ...
      {recoveryData.length > 0 && (
        <div className="rounded-[10px] border border-white/[0.06] bg-white/[0.015] p-5">
          <div className="mb-4">
            ...
            <p className="text-[11px] text-white/25 mt-0.5">HF before (red) vs after (green) each intervention</p>
          </div>
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={recoveryData} margin={{ top: 4, right: 4, left: -24, bottom: 0 }} barCategoryGap="35%">
              ...
              <Bar dataKey="before" radius={[3, 3, 0, 0]} maxBarSize={18}>
                {recoveryData.map((_, i) => <Cell key={i} fill="rgba(248,113,113,0.6)" />)}
              </Bar>
              <Bar dataKey="after" radius={[3, 3, 0, 0]} maxBarSize={18}>
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="text-[12.5px] text-white/70 truncate">{ACTION_LABEL[entry.action]}</p>
                ...
                <div className="flex items-center gap-1">
                  <span className="text-[10px] font-mono text-white/20 max-w-[72px] truncate">
                    {entry.txHash}
                  </span>
                  {entry.txHash !== "—" && (
